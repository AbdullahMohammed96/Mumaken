...
function compensateScroll(rowEls, scrollbarWidths) {
	if (scrollbarWidths.left) {
		rowEls.css({
			'border-left-width': 1,
			'margin-inline-end': scrollbarWidths.left - 1
		});
	}
	if (scrollbarWidths.right) {
		rowEls.css({
			'border-right-width': 1,
			'margin-inline-start': scrollbarWidths.right - 1
		});
	}
}
		...
		for (i = 0; i < segs.length; i++) {
			...
			if (sourceSeg && sourceSeg.col === seg.col) {
				...
				seg.el.css({
					...
					'margin-inline-end': sourceEl.css('margin-inline-end'),
					'margin-inline-start': sourceEl.css('margin-inline-start')
				});
			}
		}
